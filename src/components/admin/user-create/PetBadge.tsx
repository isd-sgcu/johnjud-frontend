...
const PetBadge = (props: PetBadgeProps) => {
    ...
    return ( 
        <div className='flex w-full relative cursor-pointer select-none' onClick={handleOnClick}>
            <Icon icon={(props.value === "fromClub") ? "custom:primary:banner" : "custom:yellow:banner"} className='w-16 h-24' color={(props.value === "fromClub") ? "#08878E" : "#FFFF31"}/>
            <div className={'absolute top-[0.4rem] w-full flex flex-col items-center text-sm ' + ((props.value === "fromClub") ? "text-white" : "text-black")}>
                <Icon icon={(props.value === "fromClub") ? "custom:chess" : "ph:paw-print"} className='w-6 h-6'/>
                <div className=''>สัตว์เลี้ยง</div>
                <div className='flex flex-row items-center'>{(props.value === "fromClub") ? "ชมรม" : "ฝาก"}<Icon icon="fluent:arrow-swap-20-regular" className='w-5 h-5' hFlip={true} color={(props.value === "fromClub") ? "white" : "black"}/></div>
            </div>
        </div>
     );
}
